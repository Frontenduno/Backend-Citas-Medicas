...
import { CodigoIncorrectoException } from '../../exception/CodigoIncorrectoException';
...
export class ConfirmarCodigoUseCase {
  constructor(
  ...
  async execute(input: Input): Promise<void> {
    ...
    if (!/^\d{6}$/.test(codigo)) {
      throw new ValidacionException('El código debe contener exactamente 6 dígitos numéricos');
    }
    ...
    if (!codigoPendiente) {
      ...
      throw new CodigoExpiradoException('No hay códigos pendientes');
    }

    if (codigoPendiente.estaExpirado()) {
      const codigoExpirado = new CodigoVerificacion(
        codigoPendiente.idCodigo ?? null,
        codigoPendiente.usuarioId,
        codigoPendiente.tipo,
        codigoPendiente.codigoHash,
        'EXPIRADO',
        codigoPendiente.intentos,
        codigoPendiente.maxIntentos,
        codigoPendiente.expiraEn,
        codigoPendiente.ipSolicitud,
        codigoPendiente.userAgent,
        codigoPendiente.fechaCreacion,
        codigoPendiente.fechaUso
      );
    ...
    if (hashGenerado !== codigoPendiente.codigoHash) {
      ...
      const codigoActualizado = new CodigoVerificacion(
        codigoPendiente.idCodigo ?? null,
        codigoPendiente.usuarioId,
        codigoPendiente.tipo,
        codigoPendiente.codigoHash,
      ...
      throw new CodigoIncorrectoException(`Código incorrecto. Intentos restantes: ${codigoActualizado.intentosRestantes()}`);
    }

    // Código válido, procedemos con la transacción
    await this.transactionManager.withTransaction(async (connection) => {
      const codigoUsado = new CodigoVerificacion(
        codigoPendiente.idCodigo ?? null,
        codigoPendiente.usuarioId,
        codigoPendiente.tipo,
        codigoPendiente.codigoHash,
