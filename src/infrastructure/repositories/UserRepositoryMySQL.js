...
class UsuarioRepositoryMySQL {
  async existsByEmail(email, connection = null) {
  ...
  async create(usuario, connection = null) {
    const executor = connection || pool;
    const [result] = await executor.execute(
      `INSERT INTO Usuario (contrasena, nombres, apellidos, correo, telefono, rol) VALUES (?, ?, ?, ?, ?, ?)`,
      [
        usuario.contrasena,
        usuario.nombres,
        usuario.apellidos,
