...
export function createCitaRouter(
  citaRepository = new MySQLCitaRepository(),
): Router {
  const router = Router();
  ...
  const getCitasByPacienteUseCase = new GetCitasByPacienteUseCase(
    citaRepository,
  );
  ...
  const checkDisponibilidadUseCase = new CheckDisponibilidadUseCase(
    citaRepository,
  );
  ...
  router.get(
    "/disponibilidad/:medicoId/:fecha/:hora",
    controller.checkDisponibilidad,
  );
